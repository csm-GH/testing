...
// 2) GET User login by Name and Password
app.get('/api/users/login', (req, res) => {
  const { username, password } = req.query;
  if (!username || !password) {
    return res
      .status(400)
      .json({ error: "Missing required parameters: username and password" });
  }
  ...
  connection.query(query, [username, password], (err, results) => {
    ...
    if (results.length === 0) {
      return res.status(200).json({ msg: "User does not exist or password is incorrect" });
    }
...
app.get('/api/orders', (req, res) => {
  ...
  if (userId) {
    get.getOrderByUserId(userId, (err, orders) => {
      if (err) {
        return res.status(500).json({
      ...
      const updated = orders.map((order) => {
        if (order.ProductImage)
          order.ProductImageURL = `data:image/png;base64,${Buffer.from(
            order.ProductImage
          ).toString("base64")}`;
        else order.ProductImageURL = null;
        return order;
      });
      res.status(200).json(updated);
    });
...
app.get('/api/category-types', (req, res) => {
  ...
  if (typeId) {
    const query = "SELECT * FROM Category_type WHERE TypeID =?";
...
app.post('/api/users', (req, res) => {
  ...
  insert.insertUser(userData, (err, userId) => {
    ...
    res.status(200).json({ msg: "Successfully registered", userId });
  });
});
...
app.post('/api/products', upload.single('ProductImage'), (req, res) => {
  const productData = {
    ProductName: req.body.title || null,
    ProductStock: req.body.stock ? Number(req.body.stock) : 0,
    ProductDescription: req.body.description || null,
    ProductPrice: req.body.price ? Number(req.body.price) : 0,
  ...
  insert.insertProduct(productData, (err, productId) => {
    ...
    res.status(200).json({ message: "Product inserted successfully", productId });
  });
});
...
app.post('/api/category-type', (req, res) => {
  ...
  insert.insertCategoryType(categoryTypeData, (err, typeId) => {
    ...
    res.status(200).json({ message: "Category type inserted successfully", typeId });
  });
});
...
app.put('/api/products/update', upload.single('ProductImage'), (req, res) => {
  ...
  let updateFields = {};
  if (req.body.title) updateFields.ProductName = req.body.title;
  if (req.body.stock) updateFields.ProductStock = Number(req.body.stock);
  if (req.body.description) updateFields.ProductDescription = req.body.description;
  if (req.body.price) updateFields.ProductPrice = Number(req.body.price);
  // if (req.body.ProductImage) updateFields.ProductImage = req.body.ProductImage;
...
app.put('/api/category/update', (req, res) => {
  ...
  if (!typeId ||!TypeName) {
    return res.status(400).json({ error: "Missing required fields: TypeID and TypeName." });
  }
  const query = "UPDATE Category_type SET TypeName =? WHERE TypeID =?";
  connection.query(query, [TypeName, typeId], (err, results) => {
    if (err) {
